# Dirty Pipe SoftX disposition — tonight (15/16 Aug 2026)

**CVE:** CVE-2022-0847  
**SoftX bar:** appendix-complete without live QEMU predicates if build overruns.
...
| Why not on SoftX case-study pin | Fixed before `v6.6.47` — wrong experiment |
...
Dirty Pipe `bzImage` was **paused** (`SIGSTOP`) so C6 (SoftX hard gate: kpatch `-O2`/`-Os`) could monopolize the 4-core Lab Test Server. Resume with `/tmp/lab-resume-dirtypipe.sh` after `C6_KPATCH_OPT_DONE`.
...
Checklist **Dirty Pipe** SoftX appendix = **CLOSED** on pin+skeleton+disposition. Full Option A acceptance remains open for Paper 2 / morning continuation.
